...
// new动画函数
uint8_t HugoUI_Animation_Liner(float *a, float *a_trg, uint8_t n)
{
    if (*a != *a_trg)
    {
...
HugoUIPage_t *SetPgaeUIShow(void (*PgaeUIShow)(struct HugoUI_page *thispage, HugoUIItem_t *thisitem))
{
    pageTail->PgaeUIShow = PgaeUIShow;
    return pageTail;
}
...
void HugoUICommonListShow(HugoUIPage_t *thispage, HugoUIItem_t *thisitem)
{
    ...
    {
        HugoUI_Animation_Liner(&thispage->page_y_forlist, &thispage->page_y_forlist_trg, *Slide_space[Slide_space_Page_y_forlist_speed].val);
        HugoUI_Animation_Liner(&frame_y, &frame_y_trg, *Slide_space[Slide_space_Fre_y_speed].val);
        HugoUI_Animation_Liner(&frame_width, &frame_width_trg, *Slide_space[Slide_space_Fre_width_speed].val);
        HugoUI_Animation_Liner(&slidbar_y, &slidbar_y_trg, *Slide_space[Slide_space_Slidbar_y_speed].val);
    }
    ...
    // 绘制滑动条里的会滚动的box
    Oled_u8g2_DrawBox(thispage->page_x + 125, thispage->page_y + slidbar_y, thispage->page_x + 3, ceil((float)SCREEN_HEIGHT / thispage->itemMax));
...
void HugoUICommonIconShow(HugoUIPage_t *thispage, HugoUIItem_t *thisitem)
{
    ...
    {
        HugoUI_Animation_Liner(&thispage->page_x, &thispage->page_x_trg, *Slide_space[Slide_space_Page_x_foricon_speed].val);
        HugoUI_Animation_Liner(&frame_y, &frame_y_trg, *Slide_space[Slide_space_Fre_y_speed].val);
        HugoUI_Animation_Liner(&frame_x, &frame_x_trg, *Slide_space[Slide_space_Fre_x_speed].val);
        HugoUI_Animation_Liner(&icon_move_x, &icon_move_x_trg, 75);
        HugoUI_Animation_Liner(&icon_rectangle_x, &icon_rectangle_x_trg, 65);
    }
    else // 缓动动画标志位为False则没有缓动效果
    {
...
void HugoUI_Control(void)
{
    ...
    {
        currentPage->PgaeUIShow(currentPage, currentItem);
    }

...
/* TIM10定时中断10kHz：电流环中断源（PWM 仍为 20kHz） */
void TIM10_FOC_Init(void)
{
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1; // 必须高于 FreeRTOS 可管理上限(5)
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;
	NVIC_Init(&NVIC_InitStructure);

	// 中断使能推迟到 EasyFOC_Init() 末尾：等编码器/电流偏置/零点标定就绪后再开
...
void EasyFOC_Init(void)
{
	...
	current_limit = 1.5f;			  // A，3505额定≤0.5A，默认1.5A
	...
	// 编码器 / 电流偏置 / 电角度零点标定就绪后，再使能 10kHz 电流环中断
	TIM10_FOC_Init();
	TIM_ClearFlag(TIM10, TIM_FLAG_Update);
...
// 10kHz 电流环中断（TIM10 更新中断）：直接调用 loopFOC()，不另设 ISR 函数
